...
class PiDiscordBot(discord.Client):
    ...
    async def _generate_tts(self, text: str, channel_id: str) -> Optional[str]:
        ...
        def clean_text(raw: str) -> str:
            ...
            t = re.sub(r'`([^`]+)`', r'\1', t)
            # 3. File/folder paths → just the last name (speak naturally)
            #    /path/to/file.txt → file.txt   /path/to/folder/ → folder
            t = re.sub(r'(?:\/[\w\-\.]+)+(\/[\w\-\.]+\.[\w]+)', r'\1', t)
            t = re.sub(r'(?:\/[\w\-\.]+)+(\/[\w\-\.]+)\/?', r'\1', t)
            # 4. Remove URLs entirely
            t = re.sub(r'https?\u00253A\u00252F\u00252F[^\s]+', '', t)
            ...
            t = re.sub(r'\n\s*\n', '\n', t)
            t = re.sub(r' {2,}', ' ', t)
            # Proper sentence spacing for natural speech pacing
            t = re.sub(r'\.([A-Z])', '. \1', t)
            t = re.sub(r'\?([A-Z])', '? \1', t)
            t = re.sub(r'\!([A-Z])', '! \1', t)
            # Convert newlines to pauses (period creates natural speech break)
            t = t.replace('\n', '. ')
            t = re.sub(r'[\.\?!:;]+[\.\?!:;]', '.', t)  # clean doubled punctuation
            t = re.sub(r' {2,}', ' ', t)
            return t.strip()
